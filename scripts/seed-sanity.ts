/**
 * Seed Sanity CMS with ALL static content from the Franco and Sons Construction LLC site.
 *
 * Usage:
 *   SANITY_WRITE_TOKEN=<token> npx vite-node scripts/seed-sanity.ts
 *
 * Flags:
 *   --dry-run     Preview without writing to Sanity
 *   --type=<type>  Seed only a specific type. Options:
 *                  services, subservices, locations, faqs, process,
 *                  promotions, blogs, testimonials, beforeafter
 *
 * Examples:
 *   SANITY_WRITE_TOKEN=xxx npx vite-node scripts/seed-sanity.ts -- --dry-run
 *   SANITY_WRITE_TOKEN=xxx npx vite-node scripts/seed-sanity.ts -- --type=services
 *   SANITY_WRITE_TOKEN=xxx npx vite-node scripts/seed-sanity.ts -- --type=faqs --dry-run
 */
...
const typeFlag = process.argv.find((a) => a.startsWith("--type="))?.split("=")[1];

if (!token && !dryRun) {
  console.error("Error: SANITY_WRITE_TOKEN is required. Set it as an env var or use --dry-run.");
  process.exit(1);
}
...
function textToPortableText(paragraphs: string[]) {
  return paragraphs.map((text) => {
    ...
    const markDefs: Array<{ _type: string; _key: string }> = [];
    ...
    return {
      ...
      children: children.length
        ? children
        : [{ _type: "span" as const, _key: "s0", text: "", marks: [] as string[] }],
    };
  });
}

/** Create or skip a document. Returns true if created. */
...
const SUB_SERVICES: SubServiceData[] = [
  // Kitchen Remodeling sub-pages
  {
    parentSlug: "kitchen-remodeling",
  ...
  // Bathroom Remodeling sub-pages
  {
    parentSlug: "bathroom-remodeling",
  ...
  // Commercial Remodeling sub-pages
  {
    parentSlug: "commercial-remodeling",
...
async function main() {
  ...
  if (typeFlag) console.log(`Filter: --type=${typeFlag}`);
  console.log();

  const shouldSeed = (type: string) => !typeFlag || typeFlag === type;

  const stats: Record<string, { created: number; skipped: number }> = {};
  const track = (type: string) => {
    if (!stats[type]) stats[type] = { created: 0, skipped: 0 };
    return stats[type];
  };
  ...
  if (shouldSeed("services")) {
    ...
    const existing = await client.fetch<string[]>(`*[_type == "service"].slug.current`);
    const s = track("services");

    for (let i = 0; i < SERVICES.length; i++) {
      const svc = SERVICES[i];
      const _id = `service-${svc.slug}`;
      const skip = existing.includes(svc.slug) ? "slug already exists" : undefined;

      const created = await upsertDoc(
        {
          _id,
          _type: "service",
          ...
          icon: svc.icon,
          tier: svc.tier,
          seoContent: svc.seoContent,
          order: i,
        },
        `service: "${svc.title}"`,
        skip
      );
      created ? s.created++ : s.skipped++;
    }
  }

  // ── 2. Sub-Services (depend on services) ──
  if (shouldSeed("subservices")) {
    console.log("--- Sub-Services ---");
    const existing = await client.fetch<string[]>(`*[_type == "subService"].slug.current`);
    const s = track("subservices");

    for (const sub of SUB_SERVICES) {
      const _id = `subservice-${sub.slug}`;
      const skip = existing.includes(sub.slug) ? "slug already exists" : undefined;
      ...
      const created = await upsertDoc(
        {
          ...
          heroHeading: sub.heroHeading,
          heroDescription: sub.heroDescription,
          sections: sub.sections.map((sec, i) => ({
  ...
  if (shouldSeed("locations")) {
    ...
    const existing = await client.fetch<string[]>(`*[_type == "serviceArea"].slug.current`);
    const s = track("locations");

    for (const loc of LOCATIONS) {
      const _id = `servicearea-${loc.slug}`;
      const skip = existing.includes(loc.slug) ? "slug already exists" : undefined;
      ...
      const created = await upsertDoc(
        {
          ...
          ...(loc.seoContent ? { seoContent: loc.seoContent } : {}),
        },
  ...
  if (shouldSeed("faqs")) {
    console.log("--- FAQs ---");
    const existing = await client.fetch<string[]>(`*[_type == "faq"].question`);
    const s = track("faqs");
    ...
    for (let i = 0; i < FAQS.length; i++) {
      ...
      const slug = faq.question
        ...
        .slice(0, 60);
      const _id = `faq-${slug}`;
      const skip = existing.includes(faq.question) ? "question already exists" : undefined;
  ...
  // ── 5. Process Steps ──
  if (shouldSeed("process")) {
    console.log("--- Process Steps ---");
    const existing = await client.fetch<string[]>(`*[_type == "processStep"].title`);
    const s = track("process");

    for (const step of PROCESS_STEPS) {
      const _id = `processstep-${step.step}`;
      const skip = existing.includes(step.title) ? "title already exists" : undefined;
  ...
  if (shouldSeed("beforeafter")) {
    console.log("--- Before & After Projects ---");
    const existing = await client.fetch<string[]>(`*[_type == "beforeAfterProject"].projectId`);
    const s = track("beforeafter");

    for (const proj of BEFORE_AFTER_PROJECTS) {
      const _id = `beforeafter-${proj.id}`;
      const skip = existing.includes(proj.id) ? "projectId already exists" : undefined;

      const created = await upsertDoc(
        {
          _id,
          _type: "beforeAfterProject",
          projectId: proj.id,
          title: proj.title,
  ...
  // ── 7. Promotions ──
  if (shouldSeed("promotions")) {
    console.log("--- Promotions ---");
    const existing = await client.fetch<string[]>(`*[_type == "promotion"].promotionId`);
    const s = track("promotions");

    for (const promo of PROMOTIONS) {
      const _id = `promotion-${promo.id}`;
      const skip = existing.includes(promo.id) ? "promotionId already exists" : undefined;

      // Build applicableServices references (skip "all")
      const serviceRefs =
        promo.applicableServices[0] === "all"
          ? []
          : promo.applicableServices.map((slug, i) => ({
      ...
      const created = await upsertDoc(
        {
          ...
          applicableServices: serviceRefs.length > 0 ? serviceRefs : undefined,
          applicableLocations: promo.applicableLocations,
        },
  ...
  if (shouldSeed("blogs")) {
    ...
    for (const post of BLOG_POSTS) {
      ...
      const skip = existing.includes(post.slug) ? "slug already exists" : undefined;
  ...
  // ── 9. Testimonials ──
  if (shouldSeed("testimonials")) {
    console.log("--- Testimonials ---");
    const existing = await client.fetch<string[]>(`*[_type == "testimonial"].name`);
    const s = track("testimonials");
    ...
    for (const t of TESTIMONIALS) {
      const slug = t.name
        ...
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");
      const _id = `testimonial-${slug}`;
      const skip = existing.includes(t.name) ? "name already exists" : undefined;
  ...
  for (const [type, counts] of Object.entries(stats)) {
    console.log(`  ${type}: ${counts.created} created, ${counts.skipped} skipped`);
